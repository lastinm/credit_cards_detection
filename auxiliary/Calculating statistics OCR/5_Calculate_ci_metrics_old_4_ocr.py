...
for field, params in field_types.items():
    ...
    # PaddeleOCR
    paddle_metrics = calculate_metrics(
        subset["true_text"].tolist(),
        subset["paddleocr_text"].tolist(),
        **params
    )
    paddle_metrics.update({"Model": "PaddleOCR", "Field": field})
    
    results.extend([easyocr_metrics, trocr_metrics, paddle_metrics])
...
def analyze_with_scipy_ci(df, field_types, confidence=0.95):
    ...
    for field, params in field_types.items():
        ...
        # PaddleOCR
        paddle_metrics = calculate_metrics_with_ci(
            true_texts,
            subset["paddleocr_text"].tolist(),
            params["is_char_level"],
            confidence
        ...
        for model, metrics in [("EasyOCR", easyocr_metrics), ("TrOCR", trocr_metrics), ("PaddleOCR", paddle_metrics)]:
